...
public class StageManager : MonoBehaviour
{
    ...
    [Tooltip("각 스테이지에서 버텨야 하는 시간")]
    [SerializeField] private float timeToSurvivePerStage = 60f;
    [Tooltip("다음 스테이지로 이동시 대기 시간")]
    [SerializeField] private float delayBeforeNextStage = 3f; 
    ...
    private void Update()
    {
        if (_isLoadingNextStage || _currentLoadedStageChunk == null) // 다음 스테이지 로딩 중이거나 현재 스테이지가 없으면 타이머 업데이트 안 함
        {
            return;
        }
    ...
    {
        if (_isLoadingNextStage) return; // 이미 로딩 중이면 중복 실행 방지

        _isLoadingNextStage = true;
        _currentStageTimer = 0f; // 타이머 리셋

        Debug.Log($"스테이지 {_currentStageNumber} 클리어 {delayBeforeNextStage}초 후 다음 스테이지로 이동");
    ...
    private void LoadStageAndStartTimer(Vector2Int stageCoord)
    {
        ...
        int derivedSeed = baseWorldSeed + stageCoord.x * 7384 + stageCoord.y * 1934;
        chunkScript.Initialize(stageCoord, stageSize, stageBuildHeight, derivedSeed, terrainNoiseScale, terrainHeightMultiplier, stageSharedMaterial);
    ...
    private void MovePlayerToStageCenter(Vector3 stageBaseWorldPosition)
    {
        ...
        Vector3 spawnPosition = new Vector3(
            ...
            spawnYPosition,
            stageBaseWorldPosition.z + spawnLocalZ
        );
    ...
    // 블록 파괴 및 메시 업데이트 함수는 이전과 거의 동일하게 유지 가능
